...
function load_observations(input_file::AbstractString)
    ...
    required = ("longitude", "latitude", "salinity")
    for col in required
        if !(col in names(df))
    ...
    rows = NamedTuple{(:longitude, :latitude, :salinity), Tuple{Float64, Float64, Float64}}[]
    for row in eachrow(df)
        lon = row.longitude
        lat = row.latitude
        sal = row.salinity
        if !ismissing(lon) && !ismissing(lat) && !ismissing(sal) &&
           !isnan(lon) && !isnan(lat) && !isnan(sal)
            push!(rows, (longitude = Float64(lon), latitude = Float64(lat), salinity = Float64(sal)))
        end
    end

    if isempty(rows)
        error("No valid salinity observations found in $input_file")
    end

    obs = DataFrame(rows)
    println("Loaded $(nrow(obs)) salinity observations")
    return obs
end
...
function interpolate_salinity(input_file::AbstractString, output_file::AbstractString)
    mkpath(dirname(output_file))
    println("Interpolating salinity field from $(basename(input_file))")

    df = load_observations(input_file)
    x = Vector{Float64}(df.longitude)
    y = Vector{Float64}(df.latitude)
    f = Vector{Float64}(df.salinity)
    ...
    bath_file = ensure_bluetopo("ndbc_salinity", lon_min, lat_min, lon_max, lat_max, bath_n_lon, bath_n_lat)
    ...
    for (j, lat) in enumerate(lat_range), (i, lon) in enumerate(lon_range)
        value = fi[i, j]
        if !isnan(value)
            push!(grid, (longitude = lon, latitude = lat, salinity = value))
        end
    end
...
function main()
    if length(ARGS) < 2
        error("Usage: julia scripts/interpolate_salinity.jl INPUT.csv OUTPUT.csv")
    end
    interpolate_salinity(ARGS[1], ARGS[2])
end
